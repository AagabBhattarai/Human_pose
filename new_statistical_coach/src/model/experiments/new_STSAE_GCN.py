...
class STSAE_GCN_Block(nn.Module):
    ...
    def forward(self, x):
        res = self.residual(x)
        x = self.agcn(x)
        # Store attention maps from STSAM
        x, self.Ms1, self.Mt1 = self.stsam(x)
        x = self.mtcn(x) + res
        return self.relu(x)

    def get_attention_maps(self):
        """Returns the spatial and temporal attention maps for visualization"""
        return self.Ms1, self.Mt1
